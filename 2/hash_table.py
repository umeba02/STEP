...
class HashTable:
    ...
    def delete(self, key):
        ...
        #------------------------#
        self.change_size()
        self.check_size() # サイズが適切かどうか確認する
        
        bucket_index = calculate_hash(key) % self.bucket_size # ハッシュ値をもとに格納場所を求める
        item = self.buckets[bucket_index] # ハッシュテーブルから格納場所を取得
        
        # アイテムがNoneの場合
        if item==None:
            # 削除するべきアイテムが見つからなかったのでFalseを返す
            return False
        ...
        if item.key == key:
            ...
            pre_item=item # 「ひとつ前の要素」を保持
            item=item.next # 次の要素を見る
    ...
    def change_size(self):
        new_bucket_size=0
        # アイテム数がハッシュテーブルのサイズの70%以上
        if self.item_count >= self.bucket_size * 0.7:
            # テーブルのサイズを2倍+1にする
            new_bucket_size=int(self.bucket_size+self.bucket_size+1)
        # アイテム数がハッシュテーブルのサイズの30%以下
        elif self.item_count <= self.bucket_size * 0.3:
            # テーブルのサイズを半分にする
            new_bucket_size=int(new_bucket_size/2+1)
        # どちらでもなければなにもせずもどる
        else :
            return
        ...
        for item in self.buckets:
            while item:
                ...
                # 新しいハッシュテーブルのアイテム
                new_item=new_buckets[new_bucket_index]

                # 新しいアイテム
                new_buckets[new_bucket_index] = Item(item.key, item.value, new_item)
